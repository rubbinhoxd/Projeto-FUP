...
int buscar_livro(int *n,livro *v){
	char nome[20];
	int i;
...
int buscar_usuario(usuario *p, int *qtd_user){
	...
	senha_user[strlen(senha_user)-1] = '\0';
	for(i=0;i<*qtd_user;i++){
		if((strcmp(user, p[i].login)==0) && (strcmp(senha_user,p[i].senha)==0)){
			return i;
		}
	}
...
void emprestimo_livro(usuario *p, int *qte_user,livro *v, int *n){
	int emprestimo=buscar_livro(n,v);
	...
	if(p[user].index< 10){
		...
		printf("Usuario ja tem 10 livros cadastrados");
	}
...
livro* le_livro(livro *v, int *n){
  FILE *fp = fopen ("C:\\Users\\Rubens\\Progamas C geany\\livros.txt", "r");
   ...
   livro novo;
   while (fscanf(fp, "%s %s %f", novo.nome, novo.genero, &novo.preco) == 3) {
...
printf("4.Ver livros emprestados\n");
...
switch(op){
	...
	case 1: {
		...
		printf("Por fim, digite o valor do exemplar: ");
		scanf("%f", &novo.preco);
		novo.alugado=false;
	...
	case 3:{
		emprestimo_livro(p, qte_user, v, n);   //Emprestimo de livro
		break;
		//emprestimo_livro(p, qte_user, v, n);
	}
	case 4:
		//relatorio_livro();  //Empr̩stimo de Livro
		break;
	case 5:{
		limpar();
		char nome[200];
